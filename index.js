...
function compareSymbolsForKey(symbol1, symbol2) {
  ...
  let symbol5 = Symbol.for(symbol1);
  let symbol6 = Symbol.for(symbol2);
...
function useSymbolIterator(obj) {
  ...
  const items = [];
  for (let i = obj.current; i < obj.to; i++) {
    obj.next();
    items.push(obj.current);
  }
  return items;
...
let myObject = {
  ...
  next() {
    ...
    return this.currentIndex < this.category
